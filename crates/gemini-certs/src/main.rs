...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for isin in isins {
        ...
        let base_url = "https://www.certificatiederivati.it/db_bs_scheda_certificato.asp?isin=";
        let contents = std::fs::read_to_string(format!("data/{isin}.md"))
            .expect("");
        let mut attempts = 0;
        let response: CertificateResponse = loop {
            let prompt = format!(
                "what are the underlying stocks under the certificate {isin} based on {contents}?"
            );
